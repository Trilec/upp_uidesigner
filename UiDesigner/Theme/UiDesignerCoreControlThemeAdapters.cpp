...
Value ResolveFieldValue(const UiDesignerNode&n,const UiDesignerControlSpec&sp,const String&id,const UiDesignerTransientOverlay*o)const override{UiToggle::Style s=UiTheme::ResolveToggle(CoreRole(n));for(const auto&p:sp.theme_overrides){bool a=false;Value v=Effective(n,p,o,a);if(a)ApplyToggleField(s,p.adapter_field_id,v);}return ToggleFieldValue(s,id);}void ApplyPreviewStyle(Ctrl&ctrl,const UiDesignerNode&n,const UiDesignerControlSpec&sp,const UiDesignerTransientOverlay*o)const override{auto*c=dynamic_cast<UiToggle*>(&ctrl);if(!c)return;UiToggle::Style s=UiTheme::ResolveToggle(CoreRole(n));bool authored=false;for(const auto&p:sp.theme_overrides){bool a=false;Value v=Effective(n,p,o,a);if(a){authored=true;ApplyToggleField(s,p.adapter_field_id,v);}}if(authored || CoreRole(n)!=UiRole::Accent)c->SetCustomStyle(s);else c->ClearCustomStyle();}void EmitSetup(String&out,const String&m,const UiDesignerNode&n,const UiDesignerControlSpec&sp)const override{if(!HasAuthored(n,sp) && CoreRole(n)==UiRole::Accent)return;String v=m+"_style";out<<"\tUiToggle::Style "<<v<<" = UiTheme::ResolveToggle("<<RoleExpr(n.GetProperty("role","Standard"))<<");\n";for(const auto&p:sp.theme_overrides){int q=n.theme_overrides.Find(p.id);if(q>=0)EmitToggleField(out,v,p.adapter_field_id,n.theme_overrides.GetValue(q));}out<<"\t"<<m<<".SetCustomStyle("<<v<<");\n";}};
...
static void AddProgressOverrides(UiDesignerControlSpec& spec)
{
    ...
    Add(spec,"empty_text","Unfilled text","Text",PropertyEditorKind::Color,s.empty_text);
