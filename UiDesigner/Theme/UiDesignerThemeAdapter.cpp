...
static void ApplyBasicStyle(Ctrl& ctrl, UiDesignerRuntimeKind kind,
                            UiRole role, const BasicThemeValues& v, bool custom)
{
    if(kind == UiDesignerRuntimeKind::UiLabel) { auto *c = dynamic_cast<UiLabel*>(&ctrl); if(!c) return; auto s = UiTheme::ResolveLabel(role); s.palette.face[ST_NORMAL] = UiFill::Solid(v.face); s.palette.frame[ST_NORMAL] = v.frame; s.palette.ink[ST_NORMAL] = v.ink; s.metrics.face_enabled = v.face_enabled; s.metrics.frame_enabled = v.frame_enabled; s.metrics.frame_width = v.frame_width; if(custom) c->SetCustomStyle(s); else c->ClearCustomStyle(); }
    else if(kind == UiDesignerRuntimeKind::UiCheckBox) { auto *c = dynamic_cast<UiCheckBox*>(&ctrl); if(!c) return; auto s = UiTheme::ResolveCheckBox(role, UICHECKVIS_CLASSIC); s.palette.face[ST_NORMAL] = UiFill::Solid(v.face); s.palette.frame[ST_NORMAL] = v.frame; s.palette.ink[ST_NORMAL] = v.ink; s.metrics.face_enabled = v.face_enabled; s.metrics.frame_enabled = v.frame_enabled; s.metrics.frame_width = v.frame_width; if(custom) c->SetCustomStyle(s); else c->ClearCustomStyle(); }
    else if(kind == UiDesignerRuntimeKind::UiRadioButton) { auto *c = dynamic_cast<UiRadioButton*>(&ctrl); if(!c) return; auto s = UiTheme::ResolveRadioButton(role, UIRADIOVIS_CLASSIC); s.palette.face[ST_NORMAL] = UiFill::Solid(v.face); s.palette.frame[ST_NORMAL] = v.frame; s.palette.ink[ST_NORMAL] = v.ink; s.metrics.face_enabled = v.face_enabled; s.metrics.frame_enabled = v.frame_enabled; s.metrics.frame_width = v.frame_width; if(custom) c->SetCustomStyle(s); else c->ClearCustomStyle(); }
    else if(kind == UiDesignerRuntimeKind::UiToggle) { auto *c = dynamic_cast<UiToggle*>(&ctrl); if(!c) return; auto s = UiTheme::ResolveToggle(); s.palette.face[ST_NORMAL] = UiFill::Solid(v.face); s.palette.frame[ST_NORMAL] = v.frame; s.palette.ink[ST_NORMAL] = v.ink; s.metrics.face_enabled = v.face_enabled; s.metrics.frame_enabled = v.frame_enabled; s.metrics.frame_width = v.frame_width; if(custom) c->SetCustomStyle(s); else c->ClearCustomStyle(); }
    else if(kind == UiDesignerRuntimeKind::UiProgressBar) { auto *c = dynamic_cast<UiProgressBar*>(&ctrl); if(!c) return; auto s = UiTheme::ResolveProgressBar(role); s.track_palette.face[ST_NORMAL] = UiFill::Solid(v.face); s.track_palette.frame[ST_NORMAL] = v.frame; s.fill_palette.ink[ST_NORMAL] = v.ink; s.track_metrics.face_enabled = v.face_enabled; s.track_metrics.frame_enabled = v.frame_enabled; s.track_metrics.frame_width = v.frame_width; if(custom) c->SetCustomStyle(s); else c->ClearCustomStyle(); }
    else if(kind == UiDesignerRuntimeKind::UiSlider) { auto *c = dynamic_cast<UiSlider*>(&ctrl); if(!c) return; auto s = UiTheme::ResolveSlider(); s.track_palette.face[ST_NORMAL] = UiFill::Solid(v.face); s.track_palette.frame[ST_NORMAL] = v.frame; s.thumb_palette.face[ST_NORMAL] = UiFill::Solid(v.ink); s.track_metrics.face_enabled = v.face_enabled; s.track_metrics.frame_enabled = v.frame_enabled; s.track_metrics.frame_width = v.frame_width; if(custom) c->SetCustomStyle(s); else c->ClearCustomStyle(); }
    else if(kind == UiDesignerRuntimeKind::UiScrollBar) { auto *c = dynamic_cast<UiScrollBar*>(&ctrl); if(!c) return; auto s = UiTheme::ResolveScrollBar(); s.track_palette.face[ST_NORMAL] = UiFill::Solid(v.face); s.track_palette.frame[ST_NORMAL] = v.frame; s.thumb_palette.face[ST_NORMAL] = UiFill::Solid(v.ink); s.track_metrics.face_enabled = v.face_enabled; s.track_metrics.frame_enabled = v.frame_enabled; s.track_metrics.frame_width = v.frame_width; if(custom) c->SetCustomStyle(s); else c->ClearCustomStyle(); }
    else { auto *c = dynamic_cast<UiDropdown*>(&ctrl); if(!c) return; auto s = UiTheme::ResolveDropdown(role); s.palette.face[ST_NORMAL] = UiFill::Solid(v.face); s.palette.frame[ST_NORMAL] = v.frame; s.palette.ink[ST_NORMAL] = v.ink; s.metrics.face_enabled = v.face_enabled; s.metrics.frame_enabled = v.frame_enabled; s.metrics.frame_width = v.frame_width; if(custom) c->SetCustomStyle(s); else c->ClearCustomStyle(); }
}
...
public:
    ...
    void ApplyPreviewStyle(Ctrl& ctrl, const UiDesignerNode& node, const UiDesignerControlSpec& spec, const UiDesignerTransientOverlay* overlay) const override { const UiRole role = ParseRole(node.GetProperty("role", "Standard")); BasicThemeValues v = ResolveBasicThemeValues(kind_, role); bool authored = false; for(const UiDesignerThemeOverrideSpec& p : spec.theme_overrides) { int q = node.theme_overrides.Find(p.id); if(q < 0 && !HasThemeValue(node, overlay, p.id)) continue; authored = true; ApplyBasicThemeField(v, p.adapter_field_id, ResolveThemeValue(node, overlay, p.id, q >= 0 ? node.theme_overrides.GetValue(q) : p.default_value)); } ApplyBasicStyle(ctrl, kind_, role, v, authored || role != UiRole::Standard); }
