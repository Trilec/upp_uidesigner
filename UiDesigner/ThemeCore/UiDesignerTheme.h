...
struct UiDesignerThemeSnapshot {
    ...
    // Values last generated by the palette authoring policy. Explicit edits
    // remove ownership here; runtime still consumes only ordinary styles.
    ValueMap generated_fields;
...
public:
    ...
    {
        return preview_active_ ? preview_ : proposal_visible_ ? proposal_ : value_;
    }
    bool StageProposal(const String& id, const UiDesignerThemeSnapshot& value,
                       uint64 expected_revision, String& error);
    bool KeepProposal(const String& id, String& error);
    void DiscardProposal(const String& id = String());
    void ShowProposal(bool show);
    bool HasProposal() const { return !proposal_id_.IsEmpty(); }
    bool IsProposalVisible() const { return proposal_visible_; }
    const String& GetProposalId() const { return proposal_id_; }
